...
describe("Address", () => {
  ...
  describe("Validation", () => {
    ...
    invalidCases.forEach(({ field, value, error }) => {
      it(`Should throw an error when ${field} is invalid`, () => {
        ...
        expect(() => new Address(
          params.street, params.number, params.city, params.state, params.zipCode
        )).to.throw(error)
      })
    })
  })
  ...
  describe("Equality check", () => {
    ...
    beforeEach(() => {
      anotherAddress = new Address("Rua dos Bobos", 10, "São Paulo", "SP", "12345")
    })
    ...
    it("Should return false for different addresses", () => {
      const differentAddress = new Address(
        "456 Elm St", validAddress.number, validAddress.city,
        validAddress.state, validAddress.zipCode,
      )
