...
export class PatientController {
  ...
  async getPatientByName(req, res) {
    try {
      ...
      if (!patients.length) return res.status(404).json({ error: "No patients found with the given name" })
      res.status(200).json(patients)
    } catch (error) {
      res.status(500).json({ error: error.message })
    }
  ...
  async getPatientByBloodType(req, res) {
    try {
      ...
      if (!patients.length) return res.status(404).json({ error: "No patients found with the given blood type" })
      res.status(200).json(patients)
    } catch (error) {
      res.status(500).json({ error: error.message })
    }
